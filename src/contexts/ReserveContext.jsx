...
function ReserveContextProvider({ children }) {
  ...
  const [recentCheckOutDate, setRecentCheckOutDate] = useState('');
  ...
  const fetchStatusBookingByPropertyId = async (propertyId) => {
    try {
      const res = await reserveService.getStatusBookingByPropertyId(propertyId);
      setRecentCheckOutDate(res?.data?.recentPropertyBooking);
    } catch (err) {
      console.log(err);
    }
  };
